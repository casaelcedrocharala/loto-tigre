...
async function cargarBottoms() {
    // CAMBIO CLAVE: Filtramos por la categoría 'bottoms'
    const { data: productos, error } = await _supabase
        .from('productos')
        .select('*')
        .eq('categoria', 'bottoms');
...
function renderizarProductos(lista) {
    const grid = document.getElementById('grid-bottoms'); // Apunta al contenedor de bottoms
    if (!grid) return;
...
document.addEventListener('DOMContentLoaded', cargarBottoms);
